...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $email = filter_var(trim($_POST['pwd_input']), FILTER_SANITIZE_EMAIL);
    ...
    if ($result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            $link = "http://localhost/metadocs_V1/app/views/cambiar_pwd.php?token=$token";
        ...
        $_SESSION['no_existe'] = 'El correo electrónico que ingresaste no se encuentra en el sistema';
         header('Location: ../../app/views/recuperacion_pwd.php');
        exit();
    }
}
?>
